...
model = Sequential([
    ...
    Conv1D(filters=128, kernel_size=3, activation='relu', kernel_regularizer=l2(0.05)),
    GlobalMaxPooling1D(),
...
for train_idx, val_idx in kfold.split(X_train_padded, y_train_encoded):
    X_train, X_val = X_train_padded[train_idx], X_train_padded[val_idx]
    ...
    val_precision = precision_score(np.argmax(y_val, axis=1), y_pred_classes, average='weighted')
    val_recall = recall_score(np.argmax(y_val, axis=1), y_pred_classes, average='weighted')
    val_f1 = f1_score(np.argmax(y_val, axis=1), y_pred_classes, average='weighted')
...
print(f"Mean accuracy: {sum(fold_scores) / len(fold_scores):.4f}")
print(f"Mean precision: {sum(fold_precisions) / len(fold_precisions):.4f}")
print(f"Mean recall: {sum(fold_recalls) / len(fold_recalls):.4f}")
print(f"Mean F1-score: {sum(fold_f1_scores) / len(fold_f1_scores):.4f}")
